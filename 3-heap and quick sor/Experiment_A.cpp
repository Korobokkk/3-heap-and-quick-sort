...
#include "D_heap.h"
void experiment_A_filling_array(std::vector<int>& arr, int n, int q, int w, int type_generation_array)
{
...
void quicksort_walkthrough(std::vector<int> arr,int n, int q, int w, int type_generation)
{
	experiment_A_filling_array(arr, n, q, w, type_generation);
...
void d_heap_walkthrough(std::vector<int> arr, int n, int q, int w, int type_generation)
{
	experiment_A_filling_array(arr, n, q, w, type_generation);
...
void experiment_A()
{
	...
	for (int i = 1; i <= 3; i++)
	{
		...
		{
			std::vector<int>arr;
			quicksort_walkthrough(arr, n, q, w, i);
		}
		std::cout << "GG. i = " << i << "\n";
	}
	...
	for (int i = 1; i <= 3; i++)
	{
		...
		{
			std::vector<int>arr;
			d_heap_walkthrough(arr, n, q, w, i);
		}
		std::cout << "GG. i = " << i << "\n";
	}
